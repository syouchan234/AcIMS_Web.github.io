...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, autoLockDraft, isBiometricSupported, currentMasterPassword, newMasterPassword, masterPasswordConfirmation, onAutoLockChange, floatingActionSettings, onFloatingActionSettingsChange, onCurrentMasterPasswordChange, onNewMasterPasswordChange, onMasterPasswordConfirmationChange, onExport, onImport, onChangeMasterPassword, onBiometricSetup, onOpenTerms, onClose }) => (
  <IonModal isOpen={isOpen} onDidDismiss={onClose}>
    ...
    <IonContent><div className="settings-container">
      ...
      <div className="auto-lock-section"><h2>自動ロック</h2><IonItem className="auto-lock-toggle-item"><IonLabel>自動ロックを有効にする</IonLabel><IonToggle checked={autoLockDraft.enabled} onIonChange={(event) => onAutoLockChange({ ...autoLockDraft, enabled: event.detail.checked })} /></IonItem><IonItem className="auto-lock-duration-item" disabled={!autoLockDraft.enabled}><IonLabel>ロックまでの時間（分）</IonLabel><IonInput inputMode="numeric" max="60" min="1" onIonInput={(event) => onAutoLockChange({ ...autoLockDraft, minutes: Number(event.detail.value) || 1 })} type="number" value={autoLockDraft.minutes} /></IonItem></div>
      <div className="floating-action-settings"><h2>スクロールボタン</h2><IonItem><IonLabel>スクロールボタンを表示</IonLabel><IonToggle checked={floatingActionSettings.enabled} onIonChange={(event) => onFloatingActionSettingsChange({ ...floatingActionSettings, enabled: event.detail.checked })} /></IonItem>{floatingActionSettings.enabled && <div className="floating-action-details"><p>表示するボタンを個別に設定できます。</p><IonItem><IonLabel>上へ戻るボタンを表示</IonLabel><IonToggle checked={floatingActionSettings.showScrollTop} onIonChange={(event) => onFloatingActionSettingsChange({ ...floatingActionSettings, showScrollTop: event.detail.checked })} /></IonItem><IonItem><IonLabel>下へ移動ボタンを表示</IonLabel><IonToggle checked={floatingActionSettings.showScrollBottom} onIonChange={(event) => onFloatingActionSettingsChange({ ...floatingActionSettings, showScrollBottom: event.detail.checked })} /></IonItem></div>}</div>
