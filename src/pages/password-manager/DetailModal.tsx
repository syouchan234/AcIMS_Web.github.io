...
const DetailModal: React.FC<DetailModalProps> = ({ password, isPasswordVisible, onTogglePasswordVisibility, onCopy, onEdit, onDelete, onClose }) => {
  const renderField = (label: string, value: string) => <div className="detail-field"><h3>{label}</h3><div className="detail-value"><p>{value}</p><IonButton aria-label={`${label}をコピー`} disabled={!value} fill="clear" onClick={() => onCopy(value, label)} size="small" title={`${label}をコピー`}><IonIcon icon={copyOutline} slot="icon-only" /></IonButton></div></div>;
